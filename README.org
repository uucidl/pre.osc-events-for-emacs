...
#+begin_src python
  ...
  class ServerHandler(object):
	  ...
	  accept_deferred_message(self.server.client, addr, tags, data, client_address)
#+end_src
...
#+begin_src python :tangle "bridge/main.py" :results output :noweb yes
  ...
  class TestMain(unittest.TestCase):
	  ...
	  client = self.mox.CreateMock(OSCClient)
	  server = self.mox.CreateMock(OSCServer)
	  self.mox.StubOutWithMock(sys.modules[__name__], 'send_deferred_message')
...
#+begin_src elisp :tangle osc.el
  ...
  (defun osc-bridge-message-handler (msg)
	...
	(osc-send-message *grid64*client* "/grid/led/set" (nth 1 msg) (nth 2 msg) (nth 3 msg)))
