...
#+begin_src elisp
  ...
  (process-send-string
   osc-bridge-process
   (format "%S\n"
	   '(osc-packet "127.0.0.1:7016" "127.0.0.1:12002"
			("/serialosc/list" "127.0.0.1" 7016))))

  ;; additional monome
  (process-send-string
   osc-bridge-process
   (format "%S\n" '(osc-packet "127.0.0.1:7016" "127.0.0.1:10775"
			       ("/sys/info" "127.0.0.1" 7016))))
...
This shows some odd behaviour when enqueuing deferred messages.. some
messages seem lost occasionally and delivery comes often in pairs.

Instead, those lost messages arrive to emacs rather than to the
device.
