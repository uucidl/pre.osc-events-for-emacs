...
Here is the test case showing usage and expected behaviour:

#+begin_src python :tangle "bridge/selftest/test_osc.py" :results output
  import mox
  import os.path
  import sys
  import unittest
  ...
  class TestOSCBridge(unittest.TestCase):
      ...
      def test_send_relative_timestamped_message(self):
	  now_sec = 1377246142.54
	  ...
	  client = self.mox.CreateMock(OSCClient)
	  client.sendto(bundle, ("to_address", 1234))
	  time.time().AndReturn(now_sec + 0.05)
	  self.mox.ReplayAll()
...
Most of the implementation of osc.py relies on the sexpdata module to
perform sexp parsing.
