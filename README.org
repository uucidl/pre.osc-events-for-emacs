...
#+begin_src python :tangle "bridge/osc.py" :results output
  ...
  OSC.OSCTimeTag = fixedOSCTimeTag
  OSC._readTimeTag = fixed_readTimeTag
...
#+begin_src elisp :tangle osc.el
  ...
  (progn
    ...
    (osc-send-message *arc-client* '("/monome/ring/all" 0 14))
    ;; turn it off later
    (osc-send-message *arc-client* '("/monome/ring/all" 0 0) (+ (osc-timetag-in-seconds (current-time)) 2.0)))
